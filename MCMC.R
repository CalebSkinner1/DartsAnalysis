...
model_likelihood <- function(y, p) {
  ...
  combos$prob <- pmap_dbl(combos[, c("x0", "x1", "x2", "x3", "x5")],
                         function(x0, x1, x2, x3, x5) dmultinom(c(x0, x1, x2, x3, x5), prob = p))
...
mh_sampler <- function(y, alpha = c(60, 20, 9, 9, 2), burn_in = 1000, n_iter = 5000, thin = 2,
                       proposal_change = 1.5, target_accept_rate = .45, adapt_change = 50) {
  if(missing(proposal_change)){
    proposal_change <- alpha/sum(alpha) + 1# get appropriately sized jump
  }
  ...
  for (i in 1:n_iter) {
    for(j in 1:p){
      proposal <- rho_current
      proposal[j] <- rho_current[j] * runif(1, 1/proposal_change[j], proposal_change[j])
    ...
    if(i %% adapt_change == 0 & i <= burn_in){
      for(j in 1:p){
        if(accept_count[j]/adapt_change > target_accept_rate + .05){
          proposal_change[j] <- proposal_change[j] * runif(1, 1, 1.2)
        }else if(accept_count[j]/adapt_change < target_accept_rate - .05){
          proposal_change[j] <- proposal_change[j] / runif(1, 1, 1.2)
        }
      }
      accept_count <- numeric(p)
